...
public final class JavaMethod extends LispFn {
  ...
  private static boolean accept(Class javaArg, LispObject lispArg) {
    return
      (javaArg == Object.class || LispObject.class.isAssignableFrom(javaArg)) ? true : // Move this line? (with or without the LispObject part?)
  ...
  private static boolean argumentMoreSpecificThan(Class<?> a, Class<?> b) {
    return
      (a == b)                                 ? false : // Can't make a judgement now can we
      // Special: LispObject is more specific than anything else, also tricky rules
